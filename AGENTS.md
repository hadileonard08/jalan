...
- **Ownership is transferable** (`src/lib/trip-ownership.ts`, `POST …/transfer`, crown icon in the member list). It moves `saved_trips.user_id`, deletes the new owner's member row (they're identified by `user_id` now), and either keeps the outgoing owner as a Follower or removes them (`keepPreviousOwner: false`). Guards: owner-only, target must already be a member, no self-transfer. The ownership check runs **before** the membership check so a non-owner is told that, and the write is a compare-and-swap so two transfers can't both win. Test: `npx tsx scripts/test-ownership-transfer.ts`.
